...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{ Dessert.new("cookie", "100", chef) }.to raise_error
    end
  end
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      expect(dessert.eat(5)).to eq(95)
    end

    it "raises an error if the amount is greater than the quantity" do
      expect{ dessert.eat(50000) }.to raise_error
    end
  end

  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chez")
      expect(dessert.serve).to include("Chez")
    end
  end
